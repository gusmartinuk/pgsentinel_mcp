...
def register_tools(mcp: Any, config: Any | None = None) -> list[str]:
    ...
    def health_overview(server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = None
        if server:
            target = vault.get_server(server)
            if target is None:
                return {"error": f"server '{server}' not found", "available_servers": [s.name for s in vault.servers if s.enabled]}
        elif not vault.servers:
            return {"error": "no servers configured", "available_servers": []}
        else:
    ...
    def get_docker_containers(server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
    ...
    def get_container_logs(container: str, lines: int = 100, level_filter: str | None = None, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def search_logs(log_name: str, keyword: str, lines: int = 200, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_recent_errors(log_name: str, minutes: int = 60, limit: int = 100, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_postgres_health(postgres_target: str | None = None, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
    ...
    def list_allowed_tables(postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
    ...
    def describe_table(table: str, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def get_table_sample(table: str, limit: int = 20, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def diagnose_recent_failure(minutes: int = 60, server: str | None = None, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
    ...
    def query_readonly_sql(sql: str, limit: int | None = None, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def get_deployment_info(server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_container_image_info(container: str, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_container_status(container: str, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_container_restart_count(container: str, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_errors_grouped(log_name: str, minutes: int = 60, limit: int = 100, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_logs_since_deploy(container: str, limit: int = 200, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def get_recent_migrations(limit: int = 10, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def get_failed_jobs(limit: int = 20, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def get_table_row_count(table: str, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def diagnose_deployment_issue(minutes: int = 60, server: str | None = None, postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
        pg = _resolve_postgres_target(vault, postgres_target) if postgres_target else None
    ...
    def diagnose_worker_issue(log_name: str = "worker", minutes: int = 60, limit: int = 50, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
    ...
    def diagnose_database_issue(postgres_target: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        pg = _resolve_postgres_target(vault, postgres_target)
    ...
    def diagnose_http_5xx_issue(log_name: str, lines: int = 500, server: str | None = None) -> dict[str, object]:
        vault = _get_vault_or_raise()
        target = _resolve_server(vault, server)
...
def _resolve_server(vault: Any, server: str | None) -> Any:
    if server:
        target = vault.get_server(server)
        if target is None:
            names = [s.name for s in vault.servers if s.enabled]
            raise ValueError(f"server '{server}' not found. Available: {names}")
        return target
    default = vault.get_default_server()
    if default:
        return default
    names = [s.id for s in vault.servers if s.enabled]
    raise ValueError(f"specify a server. Available: {names}")
...
def _resolve_postgres_target(vault: Any, postgres_target: str | None, server: str | None = None) -> Any:
    if postgres_target:
        ...
        if target is None:
            names = [p.name for p in vault.postgres_targets if p.enabled]
            raise ValueError(f"postgres_target '{postgres_target}' not found. Available: {names}")
        return target
    default = vault.get_default_postgres_target()
    if default:
        return default
    names = [p.id for p in vault.postgres_targets if p.enabled]
    raise ValueError(f"specify a postgres_target. Available: {names}")
